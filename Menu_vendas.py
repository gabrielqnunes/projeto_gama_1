    ...
    def Run(self):
        ...
        while (continuarExecutando):
            ...
            for item in self.carrinho:
                print(f'({item["id"]}) - {item["nome"]}: {item["preco"]} x {item["quantidade"]}  = R$ {item["preco"]*item["quantidade"]}')
            print('======================================')
            ...
            if (entrada == '1'):
                ...
                while produtoSelecionado not in idsDeProdutos:
                    ...
                    produtoSelecionado = input('Digite o id do produto que você deseja adicionar no carrinho: ')
                    quantidadeDesejada = input('Digite a quantidade do produto que você deseja: ')
                    
                    for item in produtos:
                        if str(item["id"]) == produtoSelecionado:
                            quantidade = {"quantidade": int(quantidadeDesejada)}
                            item.update(quantidade)
                            self.carrinho.append(item)

            if (entrada == '2'):
                idRemovido = input('Digite o id do produto que você deseja remover no carrinho:')
                for item in self.carrinho:
